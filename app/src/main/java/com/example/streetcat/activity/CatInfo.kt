...
class CatInfo : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //catId가 intent에 넘어온 경우만 (dataChange null 값 오류 방지)
        if (intent.hasExtra("catId")) {
            ...
            catViewModel.getCatRef(catId).get().addOnSuccessListener {
                ...
                Picasso.get().load(Uri.parse(it.child("picture").value.toString()))
                    .into(cat_profile_image)
            }

            // 사용자가 고양이 즐겨찾기 등록을 했는지 안했는지에 따라 다르게 나타냄
            catViewModel.getUserRef()
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    adapter.setItemClickListener(object : CatInfoGalleryAdapter.ItemClickListener {
                        override fun onClick(view: View, position: Int) {
                            ...
                            intent.putExtra(
                                "postKey",
                                Uri.parse(catPosts[position].key).toString()
